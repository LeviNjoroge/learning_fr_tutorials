// this' one of the many REACT HOOKS that exist out there
// Use Effect is the second Hook we'll be going through in this journey of learning react
// with useEffect hook, its like telling the program to execute some code when some specific activity happens
// we will go deep into this later in this series

import React, { useState, useEffect } from 'react';

function Component1(){

    return (
        <>  
            "Hello we are learning REACT Hooks"
        </>
    );
}

export default Component1;